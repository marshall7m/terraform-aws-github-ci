...
def aws_session_expiration_check(request):
    ...
    '''
    until_exp = request.config.getoption('until_aws_exp', default=os.environ.get('UNTIL_AWS_EXP', False))
    if until_exp:
        log.info('Checking if time until AWS session token expiration meets requirement')
        ...
        if os.environ.get('AWS_SESSION_EXPIRATION', False):
            ...
            exp = datetime.strptime(os.environ['AWS_SESSION_EXPIRATION'], '%Y-%m-%dT%H:%M:%SZ')
            ...
            if until_exp_groups.group(1):
                ...
                log.debug(f'Test(s) require atleast: {until_exp} hour{"s"[:until_exp^1]}')
                log.debug(f'Time until expiration: {diff.hours} hour{"s"[:diff.hours^1]}')
                until_exp_seconds = until_exp * 60 * 60
            elif until_exp_groups.group(2):
                until_exp = int(until_exp_groups.group(2))
                log.debug(f'Test(s) require atleast: {until_exp} minute{"s"[:until_exp^1]}')
                diff_minutes = diff.seconds // 60
                log.debug(f'Time until expiration: {diff_minutes} minute{"s"[:diff_minutes^1]}')
                until_exp_seconds = until_exp * 60

            if actual_until_exp_seconds < until_exp_seconds:
                pytest.raises('AWS session token needs to be refreshed before running tests')
        else:
