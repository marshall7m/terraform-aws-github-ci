...
def create_sha256_sig(value, payload):
    ...
    return hmac.new(bytes(str(value), 'utf-8'), bytes(str(payload), 'utf-8'), hashlib.sha256).hexdigest()

@patch.dict(os.environ, {'GITHUB_WEBHOOK_SECRET_SSM_KEY': 'dummy-ssm-key'})
@patch('function.lambda_function.ssm.get_parameter', return_value = {'Parameter': {'Value': 'bar'}})
...
@pytest.mark.parametrize('header_sig,payload,github_secret,expected_msg', [
    pytest.param('sha256=' + create_sha256_sig('baz', 'foo'), 'foo', 'bar', 'Header signature and expected signature do not match', id='invalid_sha256'),
    pytest.param('sha=' + create_sha256_sig('bar', 'foo'), 'foo', 'bar', 'Signature not signed with sha256 (e.g. sha256=123456)', id='invalid_sha_type'),
    pytest.param('bar', 'foo', 'bar', 'Signature not signed with sha256 (e.g. sha256=123456)', id='invalid_sig')
])
...
@pytest.mark.parametrize('event,payload,modified_file_paths,pr_commit_message,filter_groups', [
    pytest.param(
        'pull_request', 
        {
            'repository': {'full_name': 'user/dummy-repo', 'name': 'dummy-repo', 'html_url': 'https://github.com/user/dummy-repo.git'},
            'action': 'closed',
            'merged': True,
            'pull_request': {
...
def test_matched_filter_group(mock_repo, event, payload, modified_file_paths, pr_commit_message, filter_groups):
    ...
    mock_repo.return_value.compare.return_value.files = [dotdict({'filename': path}) for path in modified_file_paths]
...
def test_no_matched_filter_group(mock_repo, event, payload, modified_file_paths, pr_commit_message, filter_groups):
...
def test_successful_lambda_handler(mock_open_file, mock_json_load, mock_json_loads, mock_validate_payload, mock_validate_sig):
...
@patch('builtins.open', new_callable=mock_open, read_data='mock_open')
@pytest.mark.parametrize('validate_sig,validate_payload', [
    pytest.param(lambda_function.ClientException('error'), None),
    pytest.param(None, lambda_function.ClientException('error'))
])
def test_error_lambda_handler(mock_open_file, mock_json_load, validate_sig, validate_payload):
